...
class ExamValuation(models.Model):
    ...
    evaluator_id = fields.Many2one('university.doctors',
                                   string='Evaluator',
        ...
        if len(search_valuation) > 1:
            raise UserError(
                _('Valuation Sheet for \n Subject --> %s \nDivision --> %s '
                  '\nExam --> %s \n is already created') % (
                    res.course_id.name, res.batch_id.name,
                    res.exam_id.college_id))
        return res
    ...
    def _compute_subject_ids(self):
        ...
        """
        for rec in self:
            rec.course_id = rec.exam_id.subject_line_ids.course_id \
